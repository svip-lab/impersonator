...
class ResNetGenerator(NetworkBase):
    ...
    def __init__(self, conv_dim=64, c_dim=5, repeat_num=9, k_size=4, n_down=2):
        ...
        # Up-Sampling
        for i in range(n_down):
            layers.append(nn.ConvTranspose2d(curr_dim, curr_dim//2, kernel_size=k_size, stride=2,
...
class ResUnetGenerator(NetworkBase):
    ...
    def __init__(self, conv_dim=64, c_dim=5, repeat_num=6, k_size=4, n_down=2):
        ...
        encoders.append(nn.Sequential(
            ...
            nn.InstanceNorm2d(conv_dim, affine=False),
            nn.ReLU(inplace=True)
        ))
...
class ImpersonatorGenerator(NetworkBase):
    ...
    def forward(self, bg_inputs, aug_bg_inputs, src_inputs, tsf_inputs, T):
        ...
        tsf_x = self.tsf_model.encoders[0](tsf_inputs) + warp
        ...
        return img_bg, aug_bg, src_img, src_mask, tsf_img, tsf_mask
    ...
    def swap(self, tsf_inputs, src_encoder_outs12, src_encoder_outs21, src_resnet_outs12, src_resnet_outs21, T12, T21):
        ...
        tsf_x = self.tsf_model.encoders[0](tsf_inputs) + self.transform(src_x12, T12) + self.transform(src_x21, T21)
    ...
    def inference(self, src_encoder_outs, src_resnet_outs, tsf_inputs, T):
        ...
        tsf_x = self.tsf_model.encoders[0](tsf_inputs) + self.transform(src_x, T)
