...
class BaseSMPL(nn.Module):
    def __init__(self, smpl_data, rotate=False):
        """
        pkl_path is the path to a SMPL model
        """
        super(BaseSMPL, self).__init__()
        self.rotate = rotate
        ...
        self.faces = dd['f'].astype(np.int32)
    ...
    def forward(self, beta, theta):
        ...
        # 1. Add shape blend shapes, Template + Bs(shape)
        ...
        # 3. Add pose blend shapes, Template + Bs(shape) + Bp(pose)
        # ------- theta    : (N, 72)
        # ------- reshape  : (N*24, 3)
        # ------- rodrigues: (N*24, 9)
        # -- Rs = reshape  : (N, 24, 3, 3)
...
class JointRegressor(nn.Module):
    def __init__(self, smpl_data, joint_type='cocoplus19'):
        super(JointRegressor, self).__init__()

        if joint_type == 'body25':
            # This returns CMU 25 keypoints: 6890 x 25
            self.joint_regressor = torch.FloatTensor(
                np.asarray(smpl_data['body25_regressor'].todense()))
        elif joint_type == 'face70':
            # This returns face 70 keypoints: 6890 x 70
            self.joint_regressor = torch.FloatTensor(
                np.asarray(smpl_data['face70_regressor'].todense()))
        elif joint_type == 'cocoplus19':
            # This returns cocoplus 19 keypoints: 19 x 6890 -> 6890 x 19
            self.joint_regressor = torch.FloatTensor(
                np.asarray(smpl_data['cocoplus_regressor'].T.todense()))
        else:
            raise ValueError('{} must in [body25, face70, cocoplus19]'.format(joint_type))
    ...
    def forward(self, verts):
        """
            ...
            Args:
              verts: 6890 x 3
            Returns:
              - joints: 6890 x 25 or 6890 x 19 joint locations depending on joint_type,
                if joint_regreesor is CMU BODY-25, it will return 6890 x 25,
                otherwise, it will return 6890 x 19.
        """

        # Get joints: (N, 6890) x (6890, K)
