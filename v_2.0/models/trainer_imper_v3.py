...
class ImperTrainer(BaseTrainerModel):
    ...
    def _create_discriminator(self):
        return networks.create_by_name('global_local', input_nc=3+self._D_cond_nc, ndf=64,
    ...
    def _optimize_G(self, fake_tsf_imgs, fake_imgs, fake_masks):
        ...
        fake_inputs = torch.cat([fake_tsf_imgs, self.tsf_inputs[:, 3:, ...]], dim=1)
        glb_outs, lcb_outs = self._D.forward(fake_inputs, self.pseudo_masks[bs:])
        self._loss_g_adv = (self._compute_loss_D(glb_outs, 0) +
                            self._compute_loss_D(lcb_outs, 0)) * self._opt.lambda_D_prob
