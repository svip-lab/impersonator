...
def main(swapper, evaluator, cam_strategy='smooth', output_dir='', visualizer=None):
    ...
    for v_name in video_names:
        ...
        for i, src_path in enumerate(src_info['images']):
            ...
            swapper.src_info = swapper.personalize(src_path, src_smpl=src_smpl)
            ...
            # 3. cross(self) - appearance transfer (for-loop)
            for cross_v_name in video_names:
                tgt_info = pair_info['source'][cross_v_name]

                cvn = cvt_name(cross_v_name)

                for t, tgt_path in enumerate(tgt_info['images']):
                    tgt_smpl = tgt_info['smpls'][t] if tgt_info['smpls'] is not None else None
                    tgt_img_name = os.path.split(src_path)[-1]
                    tgt_name = src_img_name.split('.')[0]

                    swapper.tgt_info = swapper.personalize(tgt_path, src_smpl=tgt_smpl)

                    preds = swapper.swap(src_info=swapper.src_info, tgt_info=swapper.tgt_info, target_part=opt.swap_part)

                    if visualizer is not None:
                        visualizer.vis_named_img('ref', swapper.tgt_info['image'])
                        visualizer.vis_named_img('preds', preds)

                    if output_dir:
                        # print(src_outdir + '/' + 'gt_' + cvn + '_' + tgt_img_name)
                        # print(src_outdir + '/' + 'pred_' + cvn + '_' + tgt_img_name)
                        save_image(tgt_path, src_outdir + '/' + 'gt_' + cvn + '_' + tgt_img_name)
                        im2disk(np.transpose(preds[0].cpu().numpy(), (1, 2, 0)),
                                src_outdir + '/' + 'pred_' + cvn + '_' + tgt_img_name)

                    print(v_name, src_name, cross_v_name, tgt_name)
                    break
