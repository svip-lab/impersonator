...
def plot_fim_enc(fim_enc, map_name):
    ...
    for fim_i in fim_enc:
        img = cvt_fim_enc(fim_i, map_name)
        imgs.append(img)
...
def get_f2vts(uv_mapping_path, fill_back=False):
    ...
    vts = np.concatenate([vts, np.ones((vts.shape[0], 1), dtype=np.float32)], axis=-1)
...
def par_mapping(nf, part_info, fill_back):
...
def front_face_mapping(nf, front_face_info, fill_back):
...
def create_mapping(map_name, mapping_path='pretrains/mapper.txt',
                   ...
                   front_info='pretrains/front_face_2.json',
    ...
    if map_name == 'uv':
        ...
        bg = np.array([[1, 1, 0]], dtype=np.float32)
    elif map_name == 'par':
        map_fn, bg = par_mapping(nf, part_info, fill_back)
    elif map_name == 'par_v2':
        map_fn, bg = par_mapping(nf, part_info_2, fill_back)
    elif map_name == 'front':
        map_fn, bg = front_face_mapping(nf, front_info, fill_back)
    elif map_name == 'head':
        map_fn, bg = front_face_mapping(nf, head_info, fill_back)
    elif map_name == 'ids':
