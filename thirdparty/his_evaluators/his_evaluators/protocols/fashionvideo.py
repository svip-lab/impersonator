...
class FashionVideoProtocol(Protocol):
    ...
    def __init__(self, data_dir="/home/piaozx/liuwen/p300/human_pose/processed"):
        ...
        self.processed_dir = os.path.join(data_dir, "processed", "val")
        self.train_ids_file = "train.txt"
        self.test_ids_file = "val.txt"
        self.eval_path = "fashionvideo_protocol.json"
        ...
        full_eval_path = os.path.join(
            ...
            "data", "fashionvideo_protocol.json"
        )
...
class NovelViewFashionVideoProtocol(FashionVideoProtocol):
    def __init__(self, data_dir="/home/piaozx/liuwen/p300/human_pose/processed"):
        super().__init__()
        ...
        self.eval_path = "fashionvideo_protocol.json"
        """
        "PeopleSnapshot_female-3-casual_1604_0093": {
          "source": ["000.jpg", "035.jpg", "091.jpg", "120.jpg", "155.jpg", "195.jpg", "219.jpg", "251.jpg"],
          "view angle": [0, 45, 90, 135, 180, 225, 270, 315],
          "s_n" : {
              "1": ["000.jpg"],
          ...
          "novel view": false,
          "self_imitation": {
            "target": "PeopleSnapshot_female-3-casual_1604_0093",
            "range": [0, 300]
          },
          "cross_imitation": {
            "target": "PeopleSnapshot_female-3-casual_1604_0093",
        ...
        self.vid_names = []
        for name in self.eval_info.keys():
            if self.eval_info[name]["novel view"]:
    ...
    def __getitem__(self, item):
        ...
        Returns:
            eval_info (dict): the information for evaluation, it contains:
                ...
                --source (dict):
                    ...
                    --images (list of str):
                    --masks (list of str):
                    --views (np.ndarray): (ns, 6)
                    --smpls (np.ndarray): (ns, 85)

                --self_imitation (dict):
                    --images (list of str):
                    --masks (list of str):
                    --views (np.ndarray): (nv, 6), here nv is the number of views.
        ...
        vid_name = self.vid_names[item]
        vid_info = self.eval_info[vid_name]
        vid_smpls = self.get_smpls(vid_name)
        vid_kps = self.get_kps(vid_name)

        masks_names = vid_info["mask"]
        mask_img_ids = [int(t.split("_")[-1].split(".")[0]) for t in masks_names]

        vid_img_path = os.path.join(self.processed_dir, vid_name, "images")
        vid_msk_path = os.path.join(self.processed_dir, vid_name, "pred_masks")

        images = []
        masks = []

        for name in masks_names:
            img_path = os.path.join(vid_img_path, name)
            msk_path = os.path.join(vid_msk_path, "mask_" + name)

            images.append(img_path)
            masks.append(msk_path)

        views = vid_smpls[mask_img_ids, 0:6]
        smpls = vid_smpls[mask_img_ids]
        kps = vid_kps[mask_img_ids]
