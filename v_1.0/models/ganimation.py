...
class GANimation(BaseModel):
    ...
    def _init_losses(self):
        ...
        # init losses G
        self._loss_g_fake = self._Tensor([0])
        self._loss_g_cond = self._Tensor([0])
        self._loss_g_cyc = self._Tensor([0])
        self._loss_g_mask_1 = self._Tensor([0])
        self._loss_g_mask_2 = self._Tensor([0])
        self._loss_g_idt = self._Tensor([0])
        self._loss_g_masked_fake = self._Tensor([0])
        self._loss_g_masked_cond = self._Tensor([0])
        self._loss_g_mask_1_smooth = self._Tensor([0])
        self._loss_g_mask_2_smooth = self._Tensor([0])
        self._loss_rec_real_img_rgb = self._Tensor([0])
        self._loss_g_fake_imgs_smooth = self._Tensor([0])
        self._loss_g_unmasked_rgb = self._Tensor([0])

        # init losses D
        self._loss_d_real = self._Tensor([0])
        self._loss_d_cond = self._Tensor([0])
    ...
    def _gradinet_penalty_D(self, fake_imgs_masked):
        ...
        interpolated = alpha * self._real_img.data + (1 - alpha) * fake_imgs_masked.data
