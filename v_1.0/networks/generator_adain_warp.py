...
class ResUnetAdaINGenerator(NetworkBase):
    ...
    def __init__(self, conv_dim=64, c_dim=5, repeat_num=6, k_size=4, n_down=2):
        ...
        encoders = []
        encoders.append(AdaINBlock(nn.Conv2d(c_dim, conv_dim, kernel_size=7, stride=1, padding=3, bias=False)))
    ...
    def forward(self, x, encode_outs, resnets_outs):
        ...
        for i in range(self.repeat_num):
            resnet_out = self.resnets[i](resnet_out, resnets_outs[i])
...
class ImpersonatorGenerator(NetworkBase):
    ...
    def __init__(self, bg_dim, src_dim, tsf_dim, conv_dim=64, repeat_num=6):
        ...
        self.bg_model = BGGenerator(conv_dim=conv_dim, c_dim=bg_dim, repeat_num=repeat_num, k_size=3, n_down=4)
    ...
    def inference(self, src_encoder_outs, src_resnet_outs, tsf_inputs, T):
        ...
        tsf_x = self.tsf_model.encoders[0](tsf_inputs, src_x) + warp
...
if __name__ == '__main__':
    ...
    #
    # src_x = torch.rand(2, 128, 64, 64)
    # tsf_x = torch.rand(2, 128, 64, 64)
    #
    # style_x = adain(src_x, tsf_x)
    #
    # ipdb.set_trace()
