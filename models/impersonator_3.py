...
class Impersonator(BaseModel):
    ...
    def set_input_cond(self):
        ...
        input_src_cond = torch.cat([self._input_src_cond, self._input_src_cond], dim=0)
        input_desired_cond = torch.cat([self._input_src_cond, self._input_desired_cond], dim=0)

        self._input_src_cond = input_src_cond
        self._input_desired_cond = input_desired_cond
        self._input_D = torch.cat([self._input_real_imgs, input_src_cond, input_desired_cond], dim=1)
        self._j2d = torch.cat([src_info['j2d'], tgt_info['j2d']], dim=0)
    ...
    def optimize_parameters(self, trainable=True, keep_data_for_visuals=False):
        if self._is_train:
            ...
            # run
            fake_imgs, fake_masks = self.forward(keep_data_for_visuals=keep_data_for_visuals)

            loss_G = self._optimize_G(fake_imgs, fake_masks)
            ...
            # train D
            if trainable:
                loss_D = self._optimize_D(fake_imgs)
                self._optimizer_D.zero_grad()
                loss_D.backward()
                self._optimizer_D.step()

    def _optimize_G(self, fake_imgs, fake_masks):
        # D(G(Ic1, c2)*M, c2) masked
        fake_input_D = torch.cat([fake_imgs, self._input_src_cond, self._input_desired_cond], dim=1)
        ...
        if self._opt.use_face:
            self._loss_g_face = self._criterion_face(fake_imgs, self._input_real_imgs,
